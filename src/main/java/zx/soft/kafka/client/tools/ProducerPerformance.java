...
public class ProducerPerformance {
	...
	private static ArgumentParser argParser() {
		...
		parser.addArgument("--num-records").action(store()).required(true).type(Long.class)
				.metavar("NUM-RECORDS").dest("numRecords").help("number of messages to produce");

		parser.addArgument("--record-size").action(store()).required(true).type(Integer.class)
				.metavar("RECORD-SIZE").dest("recordSize").help("message size in bytes");
	...
	private static class Stats {
		...
		public void printWindow() {
			...
			double recsPerSec = 1000.0 * windowCount / ellapsed;
			double mbPerSec = 1000.0 * this.windowBytes / ellapsed / (1024.0 * 1024.0);
			System.out
					.printf("%d records sent, %.1f records/sec (%.2f MB/sec), %.1f ms avg latency, %.1f max latency.\n",
		...
		public void printTotal() {
			...
			System.out
					.printf("%d records sent, %f records/sec (%.2f MB/sec), %.2f ms avg latency, %.2f ms max latency, %d ms 50th, %d ms 95th, %d ms 99th, %d ms 99.9th.\n",

...
public class NetworkClientTest {
	...
	private NetworkClient client = new NetworkClient(selector, metadata, "mock", Integer.MAX_VALUE,
			reconnectBackoffMsTest, 64 * 1024, 64 * 1024, requestTimeoutMs, time);

	private NetworkClient clientWithStaticNodes = new NetworkClient(selector,
			new ManualMetadataUpdater(Arrays.asList(node)), "mock-static", Integer.MAX_VALUE, 0,
	...
	public void testSendToUnreadyNode() {
		RequestSend send = new RequestSend("5", client.nextRequestHeader(ApiKeys.METADATA),
	...
	public void testClose() {
		...
		ProduceRequest produceRequest = new ProduceRequest((short) 1, 1000,
				Collections.<TopicPartition, ByteBuffer> emptyMap());
		...
		assertEquals("There should be no in-flight request after close", 0,
				client.inFlightRequestCount(node.idString()));
	...
	private void checkSimpleRequestResponse(NetworkClient networkClient) {
		...
		assertEquals("Should be correlated to the original request", request,
	...
	public void testLeastLoadedNode() {
		...
		client.poll(1, time.milliseconds());
		assertFalse("After we forced the disconnection the client is no longer ready.",
				client.ready(node, time.milliseconds()));
		leastNode = client.leastLoadedNode(time.milliseconds());

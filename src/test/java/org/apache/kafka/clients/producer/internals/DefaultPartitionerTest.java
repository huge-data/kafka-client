...
public class DefaultPartitionerTest {
	...
	private List<PartitionInfo> partitions = asList(
			new PartitionInfo(topic, 1, null, nodes, nodes), new PartitionInfo(topic, 2, node1,
					nodes, nodes), new PartitionInfo(topic, 0, node0, nodes, nodes));
	...
	public void testKeyPartitionIsStable() {
		...
		assertEquals("Same key should yield same partition", partition,
				partitioner.partition("test", null, keyBytes, null, null, cluster));
	}
	...
	public void testRoundRobinWithUnavailablePartitions() {
		...
		for (int i = 1; i <= 100; i++) {
			...
			assertTrue("We should never choose a leader-less node in round robin", part == 0
					|| part == 2);
			if (part == 0)
				countForPart0++;
			else

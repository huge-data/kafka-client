...
public class RangeAssignorTest {
	...
	public void testOneConsumerOneTopic() {
		...
		assertEquals(Collections.singleton(consumerId), assignment.keySet());
		assertAssignment(Arrays.asList(new TopicPartition(topic, 0), new TopicPartition(topic, 1),
	...
	public void testOneConsumerMultipleTopics() {
		...
		assertEquals(Collections.singleton(consumerId), assignment.keySet());
		assertAssignment(Arrays.asList(new TopicPartition(topic1, 0),
	...
	public void testMultipleConsumersMixedTopics() {
		...
		assertAssignment(Arrays.asList(new TopicPartition(topic1, 1),
				new TopicPartition(topic2, 0), new TopicPartition(topic2, 1)),
	...
	public void testTwoConsumersTwoTopicsSixPartitions() {
		...
		Map<String, List<TopicPartition>> assignment = assignor.assign(partitionsPerTopic,
				consumers);
		assertAssignment(Arrays.asList(new TopicPartition(topic1, 0),
				new TopicPartition(topic1, 1), new TopicPartition(topic2, 0), new TopicPartition(
						topic2, 1)), assignment.get(consumer1));
		assertAssignment(
				Arrays.asList(new TopicPartition(topic1, 2), new TopicPartition(topic2, 2)),
				assignment.get(consumer2));
	}

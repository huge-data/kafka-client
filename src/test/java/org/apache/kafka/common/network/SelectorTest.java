...
public class SelectorTest {
	...
	public void setup() throws Exception {
		...
		this.selector = new Selector(5000, this.metrics, time, "MetricGroup",
	...
	public void testConnectionRefused() throws Exception {
		...
		selector.connect(node, new InetSocketAddress("localhost", nonListeningPort), BUFFER_SIZE,
				BUFFER_SIZE);
		while (selector.disconnected().contains(node))
			selector.poll(1000L);
	...
	public void testNormalOperation() throws Exception {
		...
		while (responseCount < conns * reqs) {
			...
			for (NetworkReceive receive : selector.completedReceives()) {
				...
				assertEquals("Check that the receive has kindly been rewound", 0, receive.payload()
						.position());
				if (responses.containsKey(receive.source())) {
					assertEquals("Check the request counter",
							(int) responses.get(receive.source()), Integer.parseInt(pieces[1]));
	...
	public void testMute() throws Exception {
		...
		assertEquals("The response should not be from the muted node", "0", selector
				.completedReceives().get(0).source());
		...
		assertEquals("We should have only one response", 1, selector.completedReceives().size());
		assertEquals("The response should be from the previously muted node", "1", selector
				.completedReceives().get(0).source());
	}
	...
	private void sendAndReceive(String node, String requestPrefix, int startIndex, int endIndex)
			throws Exception {

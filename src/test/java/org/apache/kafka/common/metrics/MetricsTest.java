...
public class MetricsTest {
	...
	@Before
	public void setup() {
		this.metrics = new Metrics(config, Arrays.asList((MetricsReporter) new JmxReporter()),
				time, true);
	}
	...
	public void testMetricName() {
		MetricName n1 = new MetricName("name", "group", "description", "key1", "value1", "key2",
				"value2");
	...
	public void testSimpleStats() throws Exception {
		...
		s.add(new Percentiles(100, -100, 100, BucketSizing.CONSTANT, new Percentile(new MetricName(
				"test.median", "grp1"), 50.0), new Percentile(new MetricName("test.perc99_9",
				"grp1"), 99.9)));
		...
		assertEquals(String.format("Occurrences(0...%d) = %f", count, count / elapsedSecs), count
				/ elapsedSecs, metrics.metrics().get(new MetricName("test.occurences", "grp1"))
				.value(), EPS);
		...
		assertEquals("Max(0...9) = 9", count - 1,
		...
		assertEquals("Rate(0...9) = 1.40625", sum / elapsedSecs,
				metrics.metrics().get(new MetricName("test.rate", "grp1")).value(), EPS);
		assertEquals(String.format("Occurrences(0...%d) = %f", count, count / elapsedSecs), count
				/ elapsedSecs, metrics.metrics().get(new MetricName("test.occurences", "grp1"))
				.value(), EPS);
		assertEquals("Count(0...9) = 10", count,
	...
	public void testQuotas() {
		...
		sensor.add(new MetricName("test1.total", "grp1"), new Total(),
				new MetricConfig().quota(Quota.upperBound(5.0)));
		sensor.add(new MetricName("test2.total", "grp1"), new Total(),
				new MetricConfig().quota(Quota.lowerBound(0.0)));
		sensor.record(5.0);
		try {
			sensor.record(1.0);
	...
	public void testPercentiles() {
		int buckets = 100;
		Percentiles percs = new Percentiles(4 * buckets, 0.0, 100.0, BucketSizing.CONSTANT,
				new Percentile(new MetricName("test.p25", "grp1"), 25), new Percentile(
						new MetricName("test.p50", "grp1"), 50), new Percentile(new MetricName(
						"test.p75", "grp1"), 75));
